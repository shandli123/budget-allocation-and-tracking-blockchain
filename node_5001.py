...
def signECDSAsecp256k1(msg, decoded_private_key):
    ...
    decoded_private_key = decoded_private_key.to_bytes((decoded_private_key.bit_length() + 7) // 8, byteorder='big')
    sk = SigningKey.from_string(decoded_private_key, curve=SECP256k1)
...
def verifyECDSAsecp256k1(msg, signature, public_key):
    msgHash = sha3_256Hash(msg)
    vk = VerifyingKey(public_key, curve=SECP256k1)
    valid = vk.verify(signature, msgHash)
    return valid
...
def search(search_key):
    ...
    for blocks in blockchain.chain:
        for transaction in blocks['transactions']:
            if search_key.lower() in transaction['purpose'] or search_key.lower() in transaction['detail']:
